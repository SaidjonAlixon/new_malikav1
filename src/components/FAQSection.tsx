...
export default function FAQSection() {
  ...
  return (
    <section id="faq" className="relative w-full py-16 sm:py-24 bg-[url('/bg-light-2.png')] dark:bg-[url('/bg-dark-2.png')] bg-fixed bg-cover bg-center overflow-hidden transition-colors duration-500">
      ...
      <div className="absolute top-[30%] right-[10%] w-[500px] h-[500px] bg-blue-600/5 dark:bg-blue-600/10 rounded-full blur-[120px] pointer-events-none mix-blend-multiply dark:mix-blend-screen" />
          ...
          {faqs.map((faq, index) => {
            ...
            return (
              <motion.div
                ...
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                className={`overflow-hidden rounded-2xl border transition-all duration-300 ${isOpen ? 'bg-white dark:bg-white/[0.05] border-blue-500/30 dark:border-blue-500/30 shadow-lg dark:shadow-[0_0_30px_-10px_rgba(59,130,246,0.15)]' : 'bg-white dark:bg-black/40 border-black/5 dark:border-white/5 hover:border-black/10 dark:hover:border-white/10 shadow-sm dark:shadow-none hover:shadow-md'}`}
              >
                <button
                  onClick={() => setOpenIndex(isOpen ? null : index)}
                  className="w-full px-6 py-5 flex items-center justify-between text-left focus:outline-none group"
                >
                  <h3 className={`font-bold text-base sm:text-lg pr-8 transition-colors duration-300 ${isOpen ? 'text-blue-600 dark:text-blue-400' : 'text-gray-800 dark:text-gray-200 group-hover:text-black dark:group-hover:text-white'}`}>
                    {faq.question}
                  </h3>
                  <div className={`flex-shrink-0 w-8 h-8 rounded-full border flex items-center justify-center transition-all duration-300 bg-blue-600 border-blue-600 shadow-[0_0_15px_rgba(37,99,235,0.4)]`}>
                    <ChevronDown 
